...
cv2.imshow( "make standard deviation smaller", img_adj.astype(np.uint8) )
...
print(f'In the adjusted image, mean={img_adj.mean()}, std={img_adj.std()}')
img_adj = adjust_image_by_offset(img_adj , img.mean())
cv2.imshow( "make standard deviation larger 1", img_adj.astype(np.uint8) )
print(f'In the adjusted image, mean={img_adj.astype(np.uint8).mean()}, std={img_adj.astype(np.uint8).std()}')
...
print(f'In the adjusted image, mean={img_adj.mean()}, std={img_adj.std()}')
img_adj = adjust_image_by_offset(img_adj , img.mean())
cv2.imshow( "make standard deviation larger 2", convert_image_to_unit8(img_adj) )
print(f'In the adjusted image, mean={img_adj.astype(np.uint8).mean()}, std={img_adj.astype(np.uint8).std()}')
